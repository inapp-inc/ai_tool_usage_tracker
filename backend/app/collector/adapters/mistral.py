...
class MistralUsageAdapter:
    ...
    async def validate_api_key(self, api_token: str, **_kwargs: object) -> None:
        ...
        if status == 401:
            raise ProviderValidationError("Invalid Mistral API key.")
        if status >= 400:
            raise ProviderValidationError(f"Mistral API key validation failed (HTTP {status}).")
    ...
    async def fetch_usage(
        ...
        *,
        ...
        return [
            UsageRecord(
                vendor_event_id=f"mistral-stub-{midpoint.date()}",
                model="mistral-large",
                occurred_at=midpoint,
                input_tokens=4_800,
                output_tokens=1_600,
                estimated_cost=Decimal("0.024000"),
            )
        ]
