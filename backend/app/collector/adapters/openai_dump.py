"""Write OpenAI Admin API pull payloads to JSON files for verification."""
...
class OpenAIPullDumper:
    """Persist raw OpenAI organization API pages and parsed rows under local storage."""
    ...
    def write_raw_page(
        ...
        *,
        ...
        logger.info("OpenAI pull dump wrote %s", path)
        return path
    ...
    def write_parsed_records(self) -> Path:
        ...
        logger.info("OpenAI pull dump wrote %s (%s rows)", path, len(self._parsed_rows))
        return path
    ...
    def write_summary(
        ...
        *,
        ...
        ingested_vendor_ids: list[str] | None = None,
        skipped_duplicate_vendor_ids: list[str] | None = None,
    ) -> Path:
        self.run_dir.mkdir(parents=True, exist_ok=True)
        summary = {
            ...
            "ingested_vendor_ids": ingested_vendor_ids or [],
            "skipped_duplicate_vendor_ids": skipped_duplicate_vendor_ids or [],
        }
        ...
        logger.info(
            "OpenAI sync summary | pulled=%s ingested=%s skipped_duplicates=%s dir=%s excel=%s",
            pulled,
            ingested,
            skipped_duplicates,
            self.run_dir,
...
def create_openai_pull_dumper(
    *,
    ...
    since: datetime,
    until: datetime,
) -> OpenAIPullDumper | None:
    if not enabled:
        return None
    base_dir = Path(storage_root) / "openai-pulls"
    return OpenAIPullDumper(base_dir, tool_id=tool_id, since=since, until=until)
